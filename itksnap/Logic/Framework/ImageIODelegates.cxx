...
{
  // Store the native component type for later querying by the UI
  m_NativeComponentType = io->GetComponentTypeInNativeImage();
}

bool LoadAnatomicImageDelegate::NeedsComponentTypeChoice() const
{
  typedef itk::ImageIOBase IOB;
  return m_NativeComponentType != IOB::UCHAR &&
         m_NativeComponentType != IOB::CHAR &&
         m_NativeComponentType != IOB::USHORT &&
         m_NativeComponentType != IOB::SHORT &&
         m_NativeComponentType != IOB::FLOAT;
}

void LoadAnatomicImageDelegate
::ConfigureImageIO(GuidedNativeImageIO *io)
{
  if(m_ComponentTypeOverride != itk::IOComponentEnum::UNKNOWNCOMPONENTTYPE)
    io->SetComponentTypeOverride(m_ComponentTypeOverride);
}
...
::ConfigureImageIO(GuidedNativeImageIO *io)
{
  LoadAnatomicImageDelegate::ConfigureImageIO(io);
