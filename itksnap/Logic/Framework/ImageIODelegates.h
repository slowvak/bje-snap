...
public:
  ...
  /** Get the native component type detected during ValidateHeader */
  itk::IOComponentEnum GetNativeComponentType() const { return m_NativeComponentType; }

  /** Returns true if the native type is non-standard and a conversion choice should be offered */
  bool NeedsComponentTypeChoice() const;

  /** Set the desired component type override for loading */
  void SetComponentTypeOverride(itk::IOComponentEnum ct) { m_ComponentTypeOverride = ct; }
  itk::IOComponentEnum GetComponentTypeOverride() const { return m_ComponentTypeOverride; }

  void ConfigureImageIO(GuidedNativeImageIO *io) override;

protected:
  LoadAnatomicImageDelegate()
    : m_NativeComponentType(itk::IOComponentEnum::UNKNOWNCOMPONENTTYPE),
      m_ComponentTypeOverride(itk::IOComponentEnum::UNKNOWNCOMPONENTTYPE) {}
  virtual ~LoadAnatomicImageDelegate() {}

  itk::IOComponentEnum m_NativeComponentType;
  itk::IOComponentEnum m_ComponentTypeOverride;
};
