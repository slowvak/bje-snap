...
DeepLearningServerPanel::SetModel(DeepLearningSegmentationModel *model)
{
  ...
  // Auto-create a "Local (Auto)" server entry on first use if no server is configured
  if(m_Model->GetServer() < 0)
  {
    auto autoServer = DeepLearningServerPropertiesModel::New();
    autoServer->SetNickname("Local (Auto)");
    autoServer->SetRemoteConnection(false);
    QString venvPath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/dls_venv";
    autoServer->SetLocalPythonVEnvPath(venvPath.toStdString());
    m_Model->UpdateServerProperties(autoServer, true);
  }
...
DeepLearningServerPanel::ShowEditorDialog()
{
  ...
  m_EditorDialog->setWindowTitle(tr("Deep Learning Extension Server Configuration"));
