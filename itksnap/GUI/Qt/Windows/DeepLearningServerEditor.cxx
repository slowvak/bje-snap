...
#if defined(Q_OS_MAC)
  // macOS: appDir is Contents/MacOS, PythonModules is in Contents/Resources/PythonModules
  return QDir(appDir).filePath("../Resources/PythonModules/itksnap_dls");
#elif defined(Q_OS_WIN)
  // Windows: appDir is bin/, PythonModules is sibling directory
  return QDir(appDir).filePath("../PythonModules/itksnap_dls");
#else
  // Linux: appDir is lib/snap-VERSION/, PythonModules is subdirectory
  return QDir(appDir).filePath("PythonModules/itksnap_dls");
#endif
}
...
QString
DeepLearningServerEditor::GetVEnvPython() const
{
  QString venvPath = QString::fromStdString(m_Model->GetLocalPythonVEnvPath());
#if defined(Q_OS_WIN)
...
void
DeepLearningServerEditor::RunAutoSetup()
{
  on_btnConfigurePackages_clicked();
}
...
DeepLearningServerEditor::on_btnConfigurePackages_clicked()
{
  ...
  QStringList args;
  args << "-m" << "venv" << "--clear" << QString::fromStdString(m_Model->GetLocalPythonVEnvPath());
...
DeepLearningServerEditor::on_VEnvInstallFinished(int exitCode, QProcess::ExitStatus status)
{
  ...
  {
    QString venvPython = GetVEnvPython();
...
DeepLearningServerEditor::on_PipUpgradePipFinished(int exitCode, QProcess::ExitStatus status)
{
  if(exitCode == 0 && status == QProcess::NormalExit)
  {
    ...
    // Install itksnap-dls from bundled source if available, otherwise from PyPI
    QStringList args;
    args << "-m" << "pip" << "install";

    QString bundledPath = GetBundledPythonModulesPath();
    if(QDir(bundledPath).exists())
    {
      // Install from bundled source in editable mode
      args << "-e" << bundledPath;
    }
    else
    {
      // Fall back to PyPI
      args << "itksnap-dls>=0.1.0";
    }
...
DeepLearningServerEditor::on_PipInstallDLSFinished(int exitCode, QProcess::ExitStatus status)
{
  ...
  if(exitCode == 0 && status == QProcess::NormalExit)
  {
    ...
    // Install platform-specific dependencies (MPS+MLX on Apple Silicon, CUDA torch on NVIDIA)
    QStringList args;
    args << "-m" << "pip" << "install";

    if(IsAppleSilicon())
    {
      // Apple Silicon: install torch with MPS support + MLX acceleration
      args << "torch<=2.8.0" << "torchvision" << "torchaudio<=2.8.0" << "mlx" << "mlx-nn";
    }
    else
    {
      // NVIDIA/CPU: install torch (pip will resolve CUDA automatically if available)
      args << "torch<=2.8.0" << "torchvision" << "torchaudio<=2.8.0";
    }

    auto *pip = new PythonProcess(venvPython, args, ui->txtInstallLog, this);
    connect(pip, &PythonProcess::finished, this, &DeepLearningServerEditor::on_PipInstallPlatformDepsFinished);
    pip->start();
  }
}

void
DeepLearningServerEditor::on_PipInstallPlatformDepsFinished(int exitCode, QProcess::ExitStatus status)
{
  updateVEnvStatusDisplay();
  if(exitCode == 0 && status == QProcess::NormalExit)
  {
    ...
    // Run setup to download nnInteractive model weights
    QStringList args;
...
DeepLearningServerEditor::on_SetupDLSFinished(int exitCode, QProcess::ExitStatus status)
{
  if(exitCode == 0 && status == QProcess::NormalExit)
  {
    ...
    if(IsAppleSilicon())
      ui->txtInstallLog->append("<b>ITK-SNAP Deep Learning Server is Ready! (Apple Silicon / MPS)</b>\n");
    else
