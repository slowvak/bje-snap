    ...
    m_Model(NULL)
{
  ...
  this->HookupShortcutToAction(QKeySequence(Qt::Key_X), ui->actionCenter_on_Cursor);
...
void MainImageWindow::LoadMainImage(const QString &file)
{
  ...
  try
    ...
    // If the native type was non-standard, auto-convert based on data range
    if(del->NeedsComponentTypeChoice())
      {
      auto *mainImage = m_Model->GetDriver()->GetCurrentImageData()->GetMain();
      double imgMin = mainImage->GetImageMinNative();
      double imgMax = mainImage->GetImageMaxNative();

      // Choose type based on range: use float if outside typical 16-bit medical range
      itk::IOComponentEnum chosenType = (imgMin < -1024.0 || imgMax > 16000.0)
        ? itk::IOComponentEnum::FLOAT
        : itk::IOComponentEnum::SHORT;

      // Reload with the chosen type
      progress.reset(new ImageIOProgressDialog(this));
      progress->display();
      irProgAccum = ImageReadingProgressAccumulator::New();
      irProgAccum->AddProgressReporterCommand(progress->createCommand());

      SmartPtr<LoadMainImageDelegate> del2 = LoadMainImageDelegate::New();
      del2->Initialize(m_Model->GetDriver());
      del2->SetComponentTypeOverride(chosenType);
      IRISWarningList warnings2;
      m_Model->GetDriver()->OpenImageViaDelegate(file.toUtf8().constData(), del2, warnings2,
                                                  NULL, irProgAccum.GetPointer());
      }
    }
  catch(exception &exc)
