...
void MainControlPanel::SetModel(GlobalUIModel *model)
{
  ...
  // Initially hide the contrast group; it will be shown when a CT image is loaded
  ui->grpContrast->setVisible(false);
  ...
  // Listen for layer changes to show/hide CT contrast presets
  connectITK(m_Model, LayerChangeEvent());
...
bool MainControlPanel::isMainImageCT()
{
  if(!m_Model)
    return false;

  auto *driver = m_Model->GetDriver();
  auto *imageData = driver->GetCurrentImageData();
  if(!imageData->IsMainLoaded())
    return false;

  auto *mainImage = imageData->GetMain();
  auto mda = mainImage->GetMetaDataAccess();

  // DICOM Modality tag
  const std::string modalityTag = "0008|0060";
  if(mda.HasKey(modalityTag))
    {
    std::string modality = mda.GetValueAsString(modalityTag);
    // Trim whitespace
    while(!modality.empty() && modality.back() <= ' ')
      modality.pop_back();
    return modality == "CT";
    }

  // Heuristic: if the image intensity range spans into negative values
  // (typical for CT Hounsfield units), treat it as CT
  auto *icModel = m_Model->GetIntensityCurveModel();
  icModel->SetLayer(mainImage);
  Vector2d range = icModel->GetNativeImageRangeForCurve();
  return range[0] < -500 && range[1] > 500;
}

void MainControlPanel::updateContrastGroupVisibility()
{
  ui->grpContrast->setVisible(isMainImageCT());
}

void MainControlPanel::applyCTPreset(double minVal, double maxVal)
{
  if(!m_Model)
    return;

  auto *imageData = m_Model->GetDriver()->GetCurrentImageData();
  if(!imageData->IsMainLoaded())
    return;

  auto *icModel = m_Model->GetIntensityCurveModel();
  icModel->SetLayer(imageData->GetMain());
  icModel->GetIntensityRangeModel(IntensityCurveModel::MINIMUM)->SetValue(minVal);
  icModel->GetIntensityRangeModel(IntensityCurveModel::MAXIMUM)->SetValue(maxVal);
}

void MainControlPanel::on_btnCTBrain_clicked()
{
  applyCTPreset(0, 70);
}

void MainControlPanel::on_btnCTAbd_clicked()
{
  applyCTPreset(-100, 300);
}

void MainControlPanel::on_btnCTLung_clicked()
{
  applyCTPreset(-900, 200);
}
