...
usage(const char *progname)
{
  ...
  cout << "   " << progname << " [options] [main_image [segmentation_image]]" << endl;
...
parse(int argc, char *argv[], CommandLineRequest &argdata)
{
  ...
  else
  {
    ...
    else if (iTrailing < argc)
    {
      ...
      // Check for second trailing argument as segmentation (only if -s not provided)
      if (!parseResult.IsOptionPresent("--segmentation") && (iTrailing + 1) < argc)
      {
        argdata.fnSegmentation.push_back(DecodeFilename(argv[iTrailing + 1]));
      }
    }
